        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative w-full max-w-md">
          ...
          <form onSubmit={submit} className="glass rounded-3xl p-8 space-y-4">
            <div className="relative">
              ...
              <input required type="email" value={form.email} onChange={e => setForm({...form, email: e.target.value})} placeholder="you@email.com" className="w-full bg-black/40 border border-white/10 rounded-full pl-12 pr-5 py-3 text-sm text-white placeholder:text-white/40 focus:outline-none focus:border-[#00FF85]/50" />
            </div>
            <div className="relative">
            ...
            <button disabled={loading} className="w-full px-6 py-4 rounded-full bg-[#00FF85] text-black text-sm font-medium hover:shadow-[0_0_30px_rgba(0,255,133,0.5)] transition disabled:opacity-50 flex items-center justify-center gap-2">{loading ? 'Signing in...' : 'Sign In'} <ArrowRight size={14}/></button>
            <div className="text-center text-sm text-white/50 pt-2">New to Visthar? <Link href="/signup" className="text-[#00FF85] hover:underline">Create account</Link></div>
          </form>
        </motion.div>
      </section>
    </PageShell>
